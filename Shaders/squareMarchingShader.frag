...
vec3 stylizeForegroundCol(float terrainVal, float hardness){
    ...
    terrainVal = clamp((terrainVal - surfaceLevel) / (1 - surfaceLevel) * 3, 0, 1);
    vec2 samplePoint = vec2(terrainVal, hardness);
    samplePoint.y = clamp(samplePoint.y, 0, 1);
    samplePoint.y = samplePoint.y / 3;
    return texture(mapColors, samplePoint, 0).rgb;
}
vec3 getForegroundCol(vec4 texCols[4], vec2 coord, vec2 texCoords, vec2 texUnits){
    vec3 cols[4] = {
    lerp( stylizeForegroundCol(texCols[0].r, texCols[0].a),   sampleOreColor(coord + vec2(0, texUnits.y)), texCols[0].g),
    lerp( stylizeForegroundCol(texCols[1].r, texCols[1].a),   sampleOreColor(coord + texUnits           ), texCols[1].g),
    lerp( stylizeForegroundCol(texCols[2].r, texCols[2].a),   sampleOreColor(coord + vec2(texUnits.x, 0)), texCols[2].g),
    lerp( stylizeForegroundCol(texCols[3].r, texCols[3].a),   sampleOreColor(coord                      ), texCols[3].g)
    };
...
vec3 stylizeBackgroundCol(float terrainVal, float hardness){
    ...
    vec2 samplePoint = vec2(terrainVal, hardness);
    samplePoint.y = clamp(samplePoint.y, 0, 1);
    samplePoint.y = (samplePoint.y + 2) / 3;
    return texture(mapColors, samplePoint, 0).rgb;
}
vec3 getBackgroundCol(vec4 texCols[4], vec2 coord, vec2 texCoords, vec2 texUnits){
    vec3 cols[4] = {
    lerp( stylizeBackgroundCol(texCols[0].r, texCols[0].a),   sampleOreColor(coord + vec2(0, texUnits.y)), texCols[0].g),
    lerp( stylizeBackgroundCol(texCols[1].r, texCols[1].a),   sampleOreColor(coord + texUnits           ), texCols[1].g),
    lerp( stylizeBackgroundCol(texCols[2].r, texCols[2].a),   sampleOreColor(coord + vec2(texUnits.x, 0)), texCols[2].g),
    lerp( stylizeBackgroundCol(texCols[3].r, texCols[3].a),   sampleOreColor(coord                      ), texCols[3].g)
    };
    return bilinearFilter(cols, texCoords);
}
